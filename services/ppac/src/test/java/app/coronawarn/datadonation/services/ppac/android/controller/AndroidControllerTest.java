...
class AndroidControllerTest {
  ...
  class CreateOtpTests {
    ...
    @Test
    void testSrsOtpServiceIsCalled() throws IOException {
      ...
      ResponseEntity<OtpCreationResponse> actResponse =
              executor.executeOtpPost(buildSrsOtpPayloadWithValidNonce(password));

      assertThat(actResponse.getStatusCode()).isEqualTo(OK);
      verify(srsOtpService, times(1)).createMinuteOtp(srs.capture(), validityCaptor.capture());
      SrsOneTimePassword cptOtp = srs.getValue();

      ZonedDateTime expectedExpirationTime = ZonedDateTime.now(ZoneOffset.UTC)
              .plusMinutes(ppacConfiguration.getSrsOtpValidityInMinutes());
      ZonedDateTime actualExpirationTime = TimeUtils.getZonedDateTimeFor(cptOtp.getExpirationTimestamp());
    ...
    private SRSOneTimePasswordRequestAndroid buildSrsOtpPayloadWithValidNonce(String password) throws IOException {
      String jws = getJwsPayloadWithNonce("mFmhph4QE3GTKS0FRNw9UZCxXI7ue+7fGdqGENsfo4g=");
