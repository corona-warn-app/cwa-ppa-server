...
public abstract class PpaDataRequestConverter<U> {
  ...
  protected ExposureWindowTestResult convertToExposureWindowTestResult(PPATestResultMetadata testResult,
    ...
    exposureWindowsTestRegistrations.addAll(
        convertToExposureWindowsAtTestRegistration(testResult.getExposureWindowsUntilTestResultList(),
            true, technicalMetadata));
    exposureWindowsTestRegistrations = exposureWindowsTestRegistrations.stream().filter(Objects::nonNull)
        .collect(Collectors.toSet());
