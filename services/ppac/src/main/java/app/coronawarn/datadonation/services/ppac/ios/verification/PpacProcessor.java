...
public class PpacProcessor {
   ...
   * {@link ApiTokenData}. Second step is to validate the DeviceToken {@link DeviceToken} against the
   * Apple DeviceCheck API. Valid DeviceToken's are then stored to prevent replay attacks. Last step is to update
   * the corresponding per-Device Data (if existing or creating a new one).
   *
